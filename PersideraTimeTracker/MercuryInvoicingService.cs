    ...
    public class MercuryInvoicingService
    {
        ...
        public MercuryInvoicingService(AppSettings settings)
        {
            ...
            if (string.IsNullOrEmpty(token))
            {
                throw new InvalidOperationException(
                    "Mercury API token not configured. Open Tools > Settings to add it.");
            }
            ...
            DateTime dueDate)
        {
            ...
            var lineItems = entries
                .GroupBy(e => e.Category?.Name ?? "Development")
                .Select(g => new
                {
                    description = g.Key,
                    quantity = Math.Round(g.Sum(e => e.GetTimeElapsed().TotalHours), 2),
        ...
        /// </summary>
        public static decimal CalculateAmount(IEnumerable<TimeTrackerData> entries, decimal hourlyRate)
        {
            double totalHours = entries.Sum(e => e.GetTimeElapsed().TotalHours);
